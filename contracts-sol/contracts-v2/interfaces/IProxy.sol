...
interface IProxy {
    ...
    function registerRole(uint64 _i, uint8 _rtype, bytes memory _extra) external;
    // add a user/keeper/provider to group
    function addToGroup(uint64 _i, uint64 _gi) external;
    ...
    function recharge(uint64 _i, uint8 _ti, uint256 _money) external;
